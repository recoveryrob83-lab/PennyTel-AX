...
Context filters use the same keys as the UI: project, taskShape, ambiguity, risk, productionModel, experiment, disposition, qualityGrade, model, thinking, role, sessionMode, contextMode. Cleared filters are omitted. Grouping supports model, thinking, role, slice, candidate, productionModel, sessionMode, contextMode, localHour, dayOfWeek. A selected group only narrows the underlying-evidence list, as in the UI; it does not change the comparison cohort or acceptance costs.
