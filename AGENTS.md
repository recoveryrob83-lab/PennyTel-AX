...
The canonical bounded-worker terminal protocol is `PENNYOS_TURN_REPORT_V1`, generated by the installed external `pennyReporter` CLI (human-facing alias: Penny Reporter). Workers invoke it against the target repository with the authoritative slice and run values, for example:

```bash
pennyReporter --repo /path/to/repository --slice <sliceId> --run-type <runType> \
  --role <role> --result <result> --verification <verification> --findings <integer> \
  [--candidate <candidate>]
```

`pennyReporter` reads project identity from `pennyos/project.json` and slice identity from `pennyos/slices/<sliceId>.json`. Workers must copy its generated `PENNYOS_TURN_REPORT_V1` block verbatim as the terminal block of the final response. Workers must never hand-author or modify `receiptId` or the canonical block. A `pennyReporter` failure remains a reporting/preflight BLOCK unless Chief Engineering explicitly grants a waiver.
